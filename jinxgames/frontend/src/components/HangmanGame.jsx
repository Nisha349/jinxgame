...
const HangmanGame = () => {
  ...
  const handleGuess = (letter) => {
    if (!wordObj || isGameOver || guessedLetters.includes(letter) || wrongGuesses.includes(letter)) {
      return;
    }
  ...
  return (
    <div className="hangman-game">
      ...
      <div style={{ whiteSpace: 'pre', fontFamily: 'monospace', fontSize: '1rem', color: '#fff' }}>
      ...
      <div className="letters">
        {alphabet.map((letter) => (
          <button
            ...
            disabled={guessedLetters.includes(letter) || wrongGuesses.includes(letter) || isGameOver}
          >
            {letter}
          </button>
        ))}
      ...
      <button onClick={startNewGame} className="restart-btn">🔁 Restart</button>
    </div>
  );
};
